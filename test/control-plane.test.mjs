...
import {createControlPlane, InMemoryPrincipalRepresentationReader, InMemoryInterpretationSessionStore, ScriptedModelClient, validateProposal} from "../src/index.mjs";
...
test("continues a connected session and saves a logical session", async () => {
  ...
  assert.equal(result.gate.decision, "accept"); assert.equal(result.sessionAction, "continue"); assert.equal(store.current.id, "s1");
});
test("starts a new session after a meaningful gap without prior prose", async () => {
  ...
  assert.equal(result.sessionAction, "start_new"); assert.equal(result.context.currentInterpretationSession, null); assert.equal(store.current.id, "session-m2");
});
...
test("timeout is an explicit adapter failure", async () => { const store=new InMemoryInterpretationSessionStore({id:"s",lastMessageAt:"2026-07-17T09:58:00Z"}); const cp=createControlPlane({principalRepresentationReader:new InMemoryPrincipalRepresentationReader({cases:[]}),interpretationSessionStore:store,primaryModelClient:{complete:()=>new Promise(()=>{})},deepModelClient:{complete:()=>new Promise(()=>{})},clock,modelTimeoutMs:1}); const result=await cp.interpretMessage({message:message("m11")}); assert.equal(result.gate.decision,"fail"); });
test("proposal validation rejects unknown enums and missing meaning", () => { assert.equal(validateProposal({sessionAction:"no",understoodMeaning:"",communicativeIntent:"",referencedEntities:[],affectedCases:[],semanticChanges:[],signalCandidates:[],ambiguities:[],requiredNextMove:"x",confidenceEvidence:[],processingMetadata:{}}).ok,false); });
